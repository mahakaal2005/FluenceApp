...
class _LoginScreenState extends State<LoginScreen> {
  ...
  void _handleLogin() {
    ...
    // Mark that this is not initial load anymore
    setState(() {
      _isInitialLoad = false;
    });

    // Trigger login through AuthBloc
  ...
  Widget build(BuildContext context) {
    return BlocListener<AuthBloc, AuthState>(
      listener: (context, state) {
        if (state is AuthLoading && !_isInitialLoad) {
          // Only show loading for actual login attempts, not initial auth check
          setState(() {
            _isLoading = true;
          });
          ...
          // Only show error for actual login attempts, not initial auth check
          if (!_isInitialLoad) {
